...
\begin{enumerate}[label=\arabic*.,leftmargin=2.4em]
  ...
  \item $i_t$: investment,
  \item $k_t$: capital carried from period $t$ into period $t+1$,
...
Current capital equals undepreciated inherited capital plus current investment:
\begin{equation}
...
Each intermediate firm combines inherited capital and current labor using a Cobb--Douglas technology:
\begin{equation}
  y_t(f)=z_t k_{t-1}(f)^{\alpha}n_t(f)^{1-\alpha}.
...
\begin{equation}
  r_t^k=\alpha\Psi_t\frac{y_t}{k_{t-1}}.
...
Symmetric intermediate firms produce aggregate output using inherited capital, current labor, and current technology:
\begin{equation}
  y_t=z_t k_{t-1}^{\alpha}n_t^{1-\alpha}.
...
Current investment adds to the undepreciated capital stock:
\begin{equation}
  k_t=(1-\delta)k_{t-1}+i_t.
