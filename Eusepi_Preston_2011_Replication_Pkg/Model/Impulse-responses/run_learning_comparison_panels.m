...
if nargin==0
    ...
    output_dir=fullfile(fileparts(mfilename('fullpath')),'artifacts', ...
        'comparisons','iid_technology_output_1pct');
elseif nargin~=2
...
artifact=struct('experiment','iid_technology_output_1pct', ...
...
[ep_ee_plugin,ep_ee_state]=make_dynare_ee_learning_plugin(ep_ee_model, ...
    ...
    config.training_shock_standard_deviations.ep_ee^2);
ep_ee=make_specification('ep_ee','E&P EE',ep_ee_plugin,ep_ee_state, ...
    config.training_shock_standard_deviations.ep_ee,@report_ep_quantities,{});

ep_ih_model=load_dynare_71_linear_model(fullfile(root,'harness','models', ...
    'ep13_ih_re_linear.mod'));
...
[ep_ih_plugin,ep_ih_state]=make_dynare_ih_learning_plugin(ep_ih_model, ...
    ep_ih_learning,config.training_shock_standard_deviations.ep_ih^2);
ep_ih=make_specification('ep_ih','E&P IH',ep_ih_plugin,ep_ih_state, ...
    config.training_shock_standard_deviations.ep_ih,@report_ep_quantities,{});
...
[nk_plugin,nk_state]=make_dynare_ee_learning_plugin(nk_model, ...
    nk_ee_learning_config(config.gain), ...
    config.training_shock_standard_deviations.nk_ee^2);
nk=make_specification('nk_ee','NK EE',nk_plugin,nk_state, ...
    config.training_shock_standard_deviations.nk_ee,@report_nk_quantities, ...
    {'inflation','interest','marginal_cost'});
specifications={ep_ee,ep_ih,nk};
end

function value=make_specification(id,label,plugin,state,shock_scale,reporter,extensions)
...
impact_innovation=config.impact_output_percent/re_quantities(1,1);
...
result=struct('id',specification.id,'label',specification.label, ...
    ...
    'impact_innovation',impact_innovation,'impact_normalization', ...
    struct('variable','output','re_impact_percent',config.impact_output_percent), ...
...
legend(nexttile(layout,1),{'25th–75th percentile','Learning median','RE'}, ...
    'Location','best');
title(layout,'Technology shocks normalized to 1% RE output impact');
...
required={'gain','random_seed','draw_count','training_periods','ir_periods', ...
    'plot_periods','band_probabilities','impact_output_percent', ...
    'explosion_magnitude_percent','training_shock_standard_deviations'};
...
assert(isequal(sort(fieldnames(scales)),sort({'ep_ee';'ep_ih';'nk_ee'}))&& ...
    all(structfun(@(x) isnumeric(x)&&isscalar(x)&&isfinite(x)&&x>0,scales)), ...
    'LearningPanels:InvalidConfig','All three training shock scales are required.');
end
