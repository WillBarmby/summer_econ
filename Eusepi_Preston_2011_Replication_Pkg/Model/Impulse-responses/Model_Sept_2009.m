...
ct1 = 0; %% Control on bounds for calibrated coefficients
...
if ct1 == 0
    ...
    A0(rk, output) = 1;
    A0(rk, gamma_x) = 1;
    A0(rk, caput) = -RBC_dummy;

    %% Aggregate resource constraint
    % output_t = consumption_share * consumption_t + investment_share * investment_t
    A0(investment,investment) = 1-cy_ratio;
    A0(investment,consumption) = cy_ratio;
    A0(investment,output) = -1;
    ...
    A0(mp, mp) = -1;
    A0(mp, hours) = (1 - alpha)*external_effects;
    A0(mp, gamma_x) = -external_effects*alpha;
    A0(mp, caput) = RBC_dummy*external_effects*alpha;

    %% Capital accumulation
    % Relates current capital to investment, technology growth,
    % capacity utilization, and lagged capital.
    A0(capital,capital) = -1;
    A0(capital,investment) = ik_ratio;
    A0(capital,gamma_x) = -(1 - delta)/gamma;
    A0(capital,caput) = -RBC_dummy*delta*theta/gamma;

    %% Capacity-utilization condition
    % With variable utilization:
    %   utilization_t = rental_rate_t / (theta - 1)
    A0(caput, caput) = 1;
    A0(caput, rk) = -RBC_dummy/(theta - 1);
    ...
    if inf_H == 0
        ...
        A0(consumption,consumption) = sigma;
        A0(consumption,hours) = psi*(1 - sigma);

    else
        % Infinite-horizon consumption decision rule
        A0(consumption,consumption) = 1;
        A0(consumption,hours) = sigma^(-1)*psi*(1 - sigma);
        A0(consumption,rk) = -c_c*R_tilda;
        A0(consumption,gamma_x) = c_c*beta_tilda^(-1);
        A0(consumption,wage) = ...
    ...
    if inf_H == 0
        ...
        A{3}(consumption,rk) = -beta_tilda*R_tilda;
        A{3}(consumption,consumption) = sigma;
        A{3}(consumption,hours) = psi*(1-sigma);
        A{3}(consumption,gamma_x) = sigma;
    end
    ...
    if inf_H == 1
        ...
        A{4}(consumption,gamma_x) = beta_tilda-c_c;

        A{4}(consumption, rk) = ...
            -beta_tilda*R_tilda ...
            *(beta_tilda*sigma^(-1) - c_c);

        A{4}(consumption, wage) = ...
            c_c*beta_tilda ...
    ...
    A{5}(capital, capital) = -(1 - delta)/gamma;


    %% Lagged capital in production
    A{5}(output, capital) = -alpha;


    %% Lagged capital in the externality equation
    A{5}(mp, capital) = -external_effects*alpha;

    %% Lagged capital in the infinite-horizon consumption rule
    if inf_H == 1
        A{5}(consumption, capital) = beta_tilda^(-1)*c_c;
