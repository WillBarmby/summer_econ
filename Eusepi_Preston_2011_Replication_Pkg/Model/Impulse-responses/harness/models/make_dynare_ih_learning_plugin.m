...
assert(islogical(config.feedback) && isscalar(config.feedback) && config.feedback, ...
    'EPIH:InvalidFeedback','E&P IH learning requires belief feedback.');
assert(config.update_timing=="decide_then_update",'EPIH:InvalidUpdateTiming', ...
    'The learning engine implements decide-then-update timing.');
...
name_indices(model.shock_names,config.observed_but_excluded,'observed excluded shock');
