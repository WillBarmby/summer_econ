function files = save_nk_nominal_panels(result,output_dir,output_stem,figure_title)
%% SAVE_NK_NOMINAL_PANELS Save six-panel NK real and nominal IRFs.
% The result contract supplies four common real quantities plus inflation and
% the nominal rate. This shared writer keeps the technology- and risk-premium
% NK figures identical in layout and reporting treatment.

if ~isfolder(output_dir), mkdir(output_dir); end
mat_path = fullfile(output_dir,[output_stem '.mat']);
pdf_path = fullfile(output_dir,[output_stem '_panels.pdf']);
png_path = fullfile(output_dir,[output_stem '_panels.png']);
summary_csv = fullfile(output_dir,[output_stem '_summary.csv']);
assert(numel(result.quantity_names)==6, ...
    'EPResearch:NKNominalPanelContract', ...
    'NK nominal panels require six reported quantities.');
