function [files,metrics] = save_ep_ee_consumption_audit(artifact,output_dir)
...
metrics = struct('rate_names',{{'completion','explosive','invalid'}}, ...
    'metric_description',['conditional learning-minus-RE responses and paired ' ...
    'paper-direct-minus-archive differences'], ...
    'response_unit',"percentage points of reported response", ...
    'reported_matlab_indices',1:periods,'reported_horizons',0:periods-1, ...
...
files = struct('mat',[stem '.mat'], ...
    ...
    'diagnostics_png',[stem '_diagnostics.png']);
...
write_summary(artifact,metrics,files.summary_csv);
end

function write_summary(artifact,metrics,path)
V = numel(artifact.variants);
Q = numel(artifact.quantity_names);
rows = V*Q;
variant = strings(rows,1);
quantity = strings(rows,1);
maximum_absolute_median_learning_minus_re_wedge = NaN(rows,1);
maximum_absolute_paired_median_direct_minus_archive = NaN(rows,1);
completion_rate = NaN(rows,1);
explosive_rate = NaN(rows,1);
invalid_rate = NaN(rows,1);
projection_event_rate = NaN(rows,1);
joint_completion_rate = repmat(metrics.joint_completion_rate,rows,1);
response_unit = repmat(string(metrics.response_unit),rows,1);
first_horizon = repmat(metrics.reported_horizons(1),rows,1);
last_horizon = repmat(metrics.reported_horizons(end),rows,1);
row = 0;
for v = 1:V
    for q = 1:Q
        row = row+1;
        variant(row) = artifact.variants(v);
        quantity(row) = string(artifact.quantity_names{q});
        maximum_absolute_median_learning_minus_re_wedge(row) = max(abs( ...
            squeeze(metrics.conditional_median_learning_minus_re_irf(v,q,:))), ...
            [],'omitnan');
        maximum_absolute_paired_median_direct_minus_archive(row) = max(abs( ...
            metrics.median_direct_minus_archive_irf(q,:)),[],'omitnan');
        completion_rate(row) = metrics.rates(v,1);
        explosive_rate(row) = metrics.rates(v,2);
        invalid_rate(row) = metrics.rates(v,3);
        projection_event_rate(row) = metrics.projection_event_rate(v);
    end
end
writetable(table(variant,quantity, ...
    maximum_absolute_median_learning_minus_re_wedge, ...
    maximum_absolute_paired_median_direct_minus_archive,completion_rate, ...
    explosive_rate,invalid_rate,projection_event_rate,joint_completion_rate, ...
    response_unit,first_horizon,last_horizon),path);
end
