 ...
 %%% SET OF PARAMETERS DEFINED IN x 
     ...
     function [Y_var,Exp_R_1Q,Exp_R_3Q,Exp_gy_1Q,Exp_gy_4Q,Exp_gn_1Q,Exp_gn_4Q,...
...
Exp_Y_var = 0;
Exp_R_1Q = 0;Exp_R_3Q = 0; Exp_gy_1Q = 0;
Exp_gy_4Q = 0; Exp_gn_1Q = 0; Exp_gn_4Q = 0;
...
Exp_gy_1Q = zeros(1,size(RND_Mat,2));

Exp_gy_4Q = zeros(1,size(RND_Mat,2));

Exp_gn_1Q = zeros(1,size(RND_Mat,2));

Exp_gn_4Q = zeros(1,size(RND_Mat,2));
...
eps_H = x(5); %% inv. of elasticity of labor supply

%phi_bar = 0.01; %% marginal cost of labor participation

sigma = x(3); %% parameter in the utility function
...
beta = 0.99*gamma^(sigma-1); %% adjusted discount rate (not really used...)
 ...
 %% steady state
  ...
  yi_ratio = yk_ratio/ik_ratio;

  phiH = eps_H-(sigma-1)/sigma*psi;
  
  if x(4) == 0
      
  theta_ratio = theta/(theta-1);
...
%% HERE WE SIMULATE AGENT'FORECASTS, TO BE COMPARED WITH DATA
 ...
 %% 2. Income (GDP) forecast

 
 Exp_y_1Q = (1-alpha)*(Exp_Y_var{1}(wage,ctn_s)+Exp_Y_var{1}(hours,ctn_s))+...
                alpha*(Exp_Y_var{1}(caput,ctn_s)+Exp_Y_var{1}(rk,ctn_s)+Y_var(cap,ctn_s));
 
 Exp_y_3Q = (1-alpha)*(Exp_Y_var{3}(wage,ctn_s)+Exp_Y_var{3}(hours,ctn_s))+...
                alpha*(Exp_Y_var{3}(caput,ctn_s)+Exp_Y_var{3}(rk,ctn_s)+Exp_Y_var{3}(cap,ctn_s));
                                
 Exp_y_4Q = (1-alpha)*(Exp_Y_var{4}(wage,ctn_s)+Exp_Y_var{4}(hours,ctn_s))+...
                alpha*(Exp_Y_var{4}(caput,ctn_s)+Exp_Y_var{4}(rk,ctn_s)+Exp_Y_var{4}(cap,ctn_s));
                
 
%Exp_gy_1Q(ctn_s) = Exp_y_1Q - Y_var(output,ctn_s); % 1Q ahead output growth forecast
Exp_gy_1Q(ctn_s) = Exp_Y_var{1}(output,ctn_s) - Y_var(output,ctn_s); % 1Q ahead output growth forecast

Exp_gy_4Q(ctn_s) = Exp_y_4Q - Exp_y_3Q; % 4Q ahead output growth forecast



% 3. Hours/employment forecast. Notice that this is in levels!


Exp_gn_1Q(ctn_s) = Exp_Y_var{1}(hours,ctn_s);


Exp_gn_4Q(ctn_s) = Exp_Y_var{4}(hours,ctn_s);
 ...
 %% checking the invertibility if the R matrix
  ...
  if rcond(R_mat1) < R_mat_tol
   ...
   ct2 = 1; Y_var = 0; Exp_Y_var = 0;
            ...
            state_ini = 0; OMEGA_c_ini = 0; OMEGA_0_ini = 0;
            Exp_R_1Q = 0;Exp_R_3Q = 0; Exp_gy_1Q = 0;
            Exp_gy_4 = 0; Exp_gn_1Q = 0; Exp_gn_4Q = 0;
            return
